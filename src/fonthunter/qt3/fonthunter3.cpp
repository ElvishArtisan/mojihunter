  ...
  : QWidget(parent)
{
  ...
  font_italic_box->insertItem(tr("No"));
  font_italic_box->insertItem(tr("Yes"));
  ...
  font_sample_textedit->setTextFormat(QTextEdit::RichText);
...
void MainWidget::applyData()
{
  QFont test_font(font_family_edit->text(),font_size_spin->value(),
		  font_weights.at(font_weight_box->currentItem()),
		  font_italic_box->currentItem());
